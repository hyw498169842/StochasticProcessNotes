...
* 设取值于可数集 $S$ 的随机变量族 $X=\{X_n:n\ge0\}$ 满足：对任意 $n>0$ 和 $i_0,\ldots,i_{n-1},i,j\in{S}$，都有 $P(X_{n+1}=j|X_n=i,X_{n-1}=i_{n-1},\ldots,X_0=i_0)=P(X_{n+1}=i|X_n=j)$，则称 $X$ 是**马尔可夫链**。
  - 进一步，若 $P(X_{n+1}=i|X_n=j)=:p_{ij}$ 与 $n$ 无关，则称 $X$ 是**时齐**的。